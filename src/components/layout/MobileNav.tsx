      ...
      {menuOpen && (
        <div className="md:hidden fixed inset-0 z-40 flex flex-col justify-end">
          ...
          <div className="relative z-50 bg-surface border-t border-border rounded-t-2xl p-4 pb-[calc(1rem+env(safe-area-inset-bottom,0px))] space-y-2 animate-in slide-in-from-bottom duration-200">
            ...
            <Link href="/yonetim/gorevler" onClick={() => setMenuOpen(false)} className={cn("flex items-center gap-3 p-3 rounded-xl transition-colors min-h-[52px]", isActive('/yonetim/gorevler') ? "bg-primary/10 text-primary" : "hover:bg-muted")}>
              <FileText className="w-5 h-5" /> <span className="font-medium">Görev & Teslim</span>
            </Link>
